...
from Model import functional_gated as mF
...
class GatedMultiheadAttention(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
        ...
        if not self._qkv_same_embed_dim:
            ...
            self.qkv_gate_proj_weight = Parameter(torch.empty((3 * embed_dim, embed_dim), **factory_kwargs))
            self.dense_gate_proj_weight = Parameter(torch.empty((embed_dim, embed_dim), **factory_kwargs))
            self.final_proj_weight = Parameter(torch.empty((embed_dim, embed_dim), **factory_kwargs))
        ...
        if bias:
            ...
            self.qkv_gate_proj_bias = Parameter(torch.empty(3 * embed_dim, **factory_kwargs))
            self.dense_gate_proj_bias = Parameter(torch.empty(embed_dim, **factory_kwargs))
            self.final_proj_bias = Parameter(torch.empty(embed_dim, **factory_kwargs))
        else:
            self.register_parameter('in_proj_bias', None)
            self.register_parameter('qkv_gate_proj_bias', None)
            self.register_parameter('dense_gate_proj_bias', None)
            self.register_parameter('final_proj_bias', None)
    ...
    def _reset_parameters(self):
        if self._qkv_same_embed_dim:
            ...
            xavier_uniform_(self.qkv_gate_proj_weight)
            xavier_uniform_(self.dense_gate_proj_weight)
            xavier_uniform_(self.final_proj_weight)
    ...
    Outputs:
        ...
        if not self._qkv_same_embed_dim:
            ...
            attn_output, attn_output_weights = mF.fused_multi_head_attention_forward(
                query, key, value, memory, self.embed_dim, self.num_heads,
                self.in_proj_weight, self.in_proj_bias,
                self.qkv_gate_proj_weight, self.qkv_gate_proj_bias,
                self.dense_gate_proj_weight, self.dense_gate_proj_bias,
                self.final_proj_weight, self.final_proj_bias,
                self.bias_k, self.bias_v, self.add_zero_attn,
                self.dropout, self.out_proj.weight, self.out_proj.bias,
                training=self.training,
